...
describe('Login GraphQL', () => {
  ...
  describe('Login Query', () => {
    const query = `query {
      login (email: "leosilva@gmail.com", password: "123") {
        accessToken
        name
      }
    }`
    ...
    test('Should return an Account on valid credentials', async () => {
      ...
      expect(res.body.data.login.name).toBe('Leo')
    })
  ...
  describe('SignUp Mutation', () => {
    const query = `mutation {
      signUp (name: "Leo", email: "leosilva@gmail.com", password: "123", passwordConfirmation: "123") {
        accessToken
        name
      }
    }`
    ...
    test('Should return an Account on valid data', async () => {
      ...
      expect(res.body.data.signUp.name).toBe('Leo')
    })
